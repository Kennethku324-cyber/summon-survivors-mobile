...
func _setup_sprite():
	...
	for i in range(1, 7):
		var tex = load("res://assets/Goblin/Goblin_walk_1/Goblin_walk_" + str(i) + ".png")
		if tex:
			frames.add_frame("walk", tex)

	frames.add_animation("hurt")
	var hurt_tex = load("res://assets/Goblin/Goblin_hurt.png")
	if hurt_tex:
		frames.add_frame("hurt", hurt_tex)

	animated_sprite.sprite_frames = frames
	animated_sprite.scale = Vector2(0.378, 0.378)
...
func _draw():
	if sprite_ready:
		return
	draw_circle(Vector2.ZERO, 14, Color(0.8, 0.2, 0.2))
	draw_circle(Vector2.ZERO, 10, Color(1.0, 0.3, 0.3))
	# Eyes
	draw_circle(Vector2(-4, -4), 2, Color(1, 1, 1))
	draw_circle(Vector2(4, -4), 2, Color(1, 1, 1))
